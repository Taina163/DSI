...
--
-- Host: 127.0.0.1:3306
-- Tempo de geração: 31-Out-2023 às 13:27
...
INSERT INTO `animais` (`id`, `nome`, `sexo`, `peso`, `racas_id`) VALUES
(1, 'Bidu', 'f', 1, 1),
(2, 'Boby', 'm', 1, 1);
...
CREATE TABLE IF NOT EXISTS `proprietarios_has_animais` (
  ...
  KEY `fk_proprietarios_has_animais_proprietarios_idx` (`proprietarios_id`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

--
-- Extraindo dados da tabela `proprietarios_has_animais`
--

INSERT INTO `proprietarios_has_animais` (`proprietarios_id`, `animais_id`) VALUES
(4, 1),
(5, 2);
...
CREATE TABLE IF NOT EXISTS `racas` (
  ...
  `descricao` longtext,
  `faixa_peso` varchar(100) DEFAULT NULL,
...
(1, 'Akitaaa', 'Agora sei o que escrever.', 'Entre 34 e 50kg', 'de R$1200,00 a  R$18.000'),
...
ALTER TABLE `proprietarios_has_animais`
  ...
  ADD CONSTRAINT `fk_proprietarios_has_animais_proprietarios` FOREIGN KEY (`proprietarios_id`) REFERENCES `proprietarios` (`id`);
COMMIT;
